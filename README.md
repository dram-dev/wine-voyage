...
1. **An offline reference** — `data/wine_reference.json`, ~940 producers and 157
   appellations, generated by `scripts/build_reference.py`. Deterministic, free,
   instant, and authoritative about *where* a producer works.
...
of world coverage — on producers outside the list it resolves roughly one in
eight. That long tail is exactly what a connected API is for.
